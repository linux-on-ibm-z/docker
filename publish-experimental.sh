...
# Publish any versions of the docker image not yet pushed to jenkins4eval/jenkins
# Arguments:
...
is-published() {
    ...
    local http_code;
    http_code=$(curl $opts -q -fsL -o /dev/null -w "%{http_code}" -H "Accept: application/vnd.docker.distribution.manifest.v2+json" -H "Authorization: Bearer $TOKEN" "https://index.docker.io/v2/$REGISTRY/$IMAGE/manifests/$tag")
